...
observeEvent(input$polygon_map,{
  if(input$polygon_map == "District" & input$domain_map == "Natural Hazards"){
    ...
    upa_haz_choices =                    (list(`River flooding` = list("Expected mortality from river floods (population count)", 
                                                                       "Expected mortality from river floods (% of population)", 
                                                                       "Expected damage on builtup from river floods (hectares)", 
                                                                       "Expected damage on builtup from river floods (% of builtup)", 
                                                                       "Expected damage on agricultural land from river floods (hectares)"), 
...
#breaks defined
  ...
  breaks <- reactive({
    ...
    if(
     ...
     unique(map_data()$indicator_1) == "Expected mortality from coastal floods (% of population)" ||
      ...
      else if( unique(map_data()$indicator_1) == "Expected annual impact on builtup area due to coastal floods (% of builtup)" ||
               unique(map_data()$indicator_1) == "Expected annual impact on builtup area due to coastal floods (hectares)"
               ){
        seq(min(map_data()$value),
            max(map_data()$value),
            (max(map_data()$value)/2))
        
      } else if(unique(map_data()$indicator_1) == "Expected mortality from river floods (% of population)"||
                unique(map_data()$indicator_1) == "Expected mortality from river floods (population count)"||
                unique(map_data()$indicator_1) == "Expected damage on builtup from river floods (hectares)"||
                unique(map_data()$indicator_1) == "Expected damage on builtup from river floods (% of builtup)"||
                unique(map_data()$indicator_1) == "Expected damage on agricultural land from river floods (hectares)"
              ){
        quantile(map_data()$value, seq(0, 1, 1 / (7)), na.rm = TRUE) %>%
          unique()
        
      }else if(unique(map_data()$indicator_1) == "Builtup exposed to landslide hazard (Builtup count)"||
               unique(map_data()$indicator_1) == "Population exposed to landslide hazard (population count)"||
               unique(map_data()$indicator_1) == "Expected annual impact on builtup area due to tropical cyclone (hectares)"||
               unique(map_data()$indicator_1) == "Expected annual impact on builtup area due to tropical cyclone (% of builtup)"
              ){
