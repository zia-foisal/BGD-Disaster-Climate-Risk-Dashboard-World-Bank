...
rwi_upazila <- rwi_data$ADM3_RWI_Summary %>% 
  pivot_longer(!upazila, names_to = "indicator", values_to = "value") %>%
...
development_indicators <- read.csv("./CCDR_Dashboard/raw_data/Socioeconomic_BISD.csv", header = TRUE, sep = ",")
...
#write.csv(unique_names_data, "./CCDR_Dashboard/raw_data/legend_bgd_data_test.csv", row.names = FALSE)
#sdg_meta <- readxl::read_excel("./CCDR_Dashboard/raw_data/sdg_plus_metadata_glossary.xlsx")

unique_names_data$var_description <- data$definition[match(unique_names_data$indicator, data$indicator)]
unique_names_data$var_name <- data$indicator_1[match(unique_names_data$indicator, data$indicator)]
unique_names_data$var_description <- data$definition[match(unique_names_data$indicator, data$indicator)]
unique_names_data$var_units <- data$unit[match(unique_names_data$indicator, data$indicator)]
unique_names_data$pillar_group <- data$domain[match(unique_names_data$indicator, data$indicator)]

#Prepare the legend data using the unique names
write.csv(unique_names_data, "./CCDR_Dashboard/raw_data/legend_bgd_data_test.csv", row.names = FALSE)
writexl::write_xlsx(data,"./CCDR_Dashboard/raw_data/data_test.xlsx")
```
